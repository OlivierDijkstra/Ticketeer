...
describe('TotalOrdersStatistic', () => {
  ...
  test('renders error state when fetch fails', async () => {
    ...
    const Comp = await resolvedComponent(TotalOrdersStatistic, {
      filters: {},
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Error fetching total orders')
      ).toBeInTheDocument();
  ...
  test('renders statistic with fetched data', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Total Orders This Month')).toBeInTheDocument();
